...
const generateIndividualCPUData = () => {
  ...
  return cpus.map((cpu, index) => {
    ...
    const cores = cpu.includes("Package")
      ? 8
      : cpu.includes("IA")
      ? 4
      : cpu.includes("GT")
    ...
    return {
      ...
      usage,
      status:
        roundedTemp > 80 ? "Critical" : roundedTemp > 70 ? "Warning" : "Normal",
    };
  });
};
...
export default function CPUMonitoringPage() {
  ...
  const [uploadedCsvContent, setUploadedCsvContent] = useState<string | null>(
    null
  );
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (row && row.length > 0 && typeof row[0] === "string") {
          if (
            row[0].includes("Date") ||
            row.join(",").includes("Date,Time,UpTime,CPU")
          ) {
            headerIndex = i;
            ...
            if (typeof tempValue === "number" && tempValue > 0) {
              ...
              if (!sensor) {
                sensor = {
                  ...
                  name: sensorName,
                  temperatures: [],
                  cores: sensorName.includes("Package")
                    ? 8
                    : sensorName.includes("IA")
                    ? 4
                    : sensorName.includes("GT")
                    ? 4
                    : sensorName.includes("HDD")
                    ? 0
                    : 1,
                  usage: sensorName.includes("HDD")
                    ? 0
      ...
      const avgTemp =
        Math.round((totalTemp / allTemperatureData.length) * 10) / 10;
  ...
  const simulateDataVariation = (csvContent: string) => {
    try {
      ...
      tempColumns.forEach((sensorName) => {
        ...
        const baseTemp = sensorName.includes("HDD")
          ? 35
